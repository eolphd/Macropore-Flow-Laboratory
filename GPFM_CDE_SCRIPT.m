...
t = BTC(:,1); % (h)
C_tracer = BTC(:,2);
x = 50;%50; % (cm) Height of the chamber
q = 0.31;%0.3; % (cm/s) Steady state flow rate

    d = 18; % (cm) depth of distribution zone
...
C_tracer = [];
C_tracer = BTC(:,2);
v = 0.73; % (cm/s) velocity of the solute
Dt = 12;  %s
...
%% no PF
BTC = [];
BTC = xlsread('Data_Spectro.xlsx',2); % 1) t, 2) C, 3) V per fraction (m), 4) Flow rate (ml/s)

% % parameters for 10Jun19a (first peak fitting)
t = BTC(1:end,1); % (h)
C_tracer = BTC(:,2);
x = 50; % (cm)
v = 0.72 ; % (cm/s) velocity of the solute
C0 = 0.85;
Dt = 43; %s
